...
lftp -d -c "set ftp:ssl-force true; set ftp:ssl-protect-data true; set net:timeout 10; open -u $SFTP_USER,placeholder -p $SFTP_PORT -e 'set xfer:parallel-max 10; set mirror:use-pget-n 10; cd $REMOTE_PATH; mirror -R --delete --only-newer --exclude .git/ --exclude .github/ -P1 --parallel=10 -x changed_files.txt .' $SFTP_HOST"
